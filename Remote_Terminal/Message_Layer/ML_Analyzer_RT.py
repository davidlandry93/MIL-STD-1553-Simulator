...
class MessageLayerAnalyzerRT:
    ...
    def _construct_data_word(self, data_wd_part):
        data_part_frame = DataLinkLayerEncoderRT().build_data_word(data_wd_part)
        # future implementation of checksum here
        return data_part_frame

    def _deconstruct_command_word(self, recd_command_frame):
        recd_command_word = DataLinkLayerDecoderRT().decode_cmd_word(recd_command_frame)
    ...
    def _construct_status_word(self, rt_address):
        status_word_frame = DataLinkLayerEncoderRT().build_status_word(rt_address)
    ...
    def _analyze_command_word(self, cmd_word):
        ...
        if not rt_address == "01":
            ...
            if tr_bit == "R":
                ...
                communication_frames.append(self._construct_status_word(rt_address))
                data_count = int(cmd_word[-2:], 16)
                for i in range(data_count):
                    communication_frames.append(
                        self._construct_data_word(
                            self.lookup_memory[
                                "{0:02x}".format(int(cmd_word[3:5], 16) + i)
                            ].encode("hex")
                        )
                    )
                return communication_frames

    def interpret_incoming_frame(self, incoming_frame):
