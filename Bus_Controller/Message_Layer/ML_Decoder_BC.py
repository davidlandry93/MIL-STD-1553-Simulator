...
class MessageLayerDecoderBC:
    def _deconstruct_status_word(self, recd_status_frame):
        recd_status_word = DataLinkLayerDecoderBC().decode_status_word(
            recd_status_frame
        )
    ...
    def _deconstruct_data_word(self, receive_data_frame):
        recd_data_word = DataLinkLayerDecoderBC().decode_data_word(receive_data_frame)
        return recd_data_word

    def interpret_incoming_frame(self, received_frame):
        if received_frame[0:3] == "100":
            ...
            # print(data_word)
            return codecs.decode(data_word, "hex_codec")
