...
class MessageLayerEncoderBC:
    def construct_command_word(self, rt_address, tr_bit, sub_address, data_word_count):
        command_word = ""
    ...
    def send_message_to_RT(self, rt_address, sub_addres_or_mode_code, message):
        ...
        for i in range(0, len(message) - 1, 2):
            ...
            msg_bytes = message[i : i + 2].encode("ascii")
            print(msg_bytes)
            word = codecs.encode(msg_bytes, "hex")
            communication_frames.append(self.construct_data_word(word))
        data_word_count = "{0:02}".format(len(data_word_characters))
        communication_frames.insert(
            0,
